...
def test_get_all_data(dolar_hoy):
    ...
    for key in expected_keys:
        ...
        if key == "dolar_tarjeta":
            ...
            if "compra" in data[key]:
                assert isinstance(
                    data[key]["compra"], float
                ), f"compra for {key} should be a float"

        if "venta" in data[key]:
            assert isinstance(
                data[key]["venta"], float
            ), f"venta for {key} should be a float"
...
def test_get_blue_price(dolar_hoy):
    ...
    if price is not None:
        assert (
            "compra" in price or "venta" in price
        ), "Both 'compra' and 'venta' keys should be in the data"
        if "compra" in price:
    ...
    for tipo_dolar in TipoDolar:
        for tipo_operacion in TipoOperacion:
            ...
            if price is not None:
                assert isinstance(
                    price, float
                ), f"{tipo_operacion.value} for {tipo_dolar.value} should be a float"
...
def test_get_all_prices(dolar_hoy):
    for tipo_operacion in TipoOperacion:
        ...
        for key, value in prices.items():
            if (
                key == TipoDolar.TARJETA.value
                and tipo_operacion == TipoOperacion.COMPRA
            ):
                assert value is None, f"{tipo_operacion} for {key} should be None"
            else:
                assert isinstance(
                    value, float
                ), f"{tipo_operacion} for {key} should be a float"
